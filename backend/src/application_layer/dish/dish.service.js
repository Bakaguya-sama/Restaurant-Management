...
class DishService {
  ...
  async updateDishAvailabilityBased(dishId, isAvailable) {
    try {
      const hasIngredients = await this.dishRepository.hasDishIngredients(
        dishId
      );
  ...
  async getAllDishes(filters = {}) {
    ...
    const dishesWithAvailability = await Promise.all(
      dishes.map(async (dish) => {
        const availability = await this.checkDishAvailability(
          dish._id || dish.id
        );
        return {
  ...
  async updateDish(id, updateData) {
    ...
    if (updateData.name && updateData.name !== existingDish.name) {
      const duplicateDish = await this.dishRepository.findByName(
        updateData.name
      );
      if (duplicateDish) {
        throw new Error("Dish with this name already exists");
  ...
  async updateDishAvailability(id, isAvailable, reason = null, staffId = null) {
    ...
    if (isAvailable) {
      ...
      if (!hasIngredients) {
        throw new Error(
          `Cannot enable dish "${existingDish.name}": Please attach at least one ingredient before enabling availability`
        );
      }
      ...
      if (
        ...
        const missingList = availability.missingIngredients
          .map(
            (ing) =>
              `${ing.name} (need: ${ing.required} ${ing.unit}, have: ${ing.available})`
          )
          .join(", ");
        throw new Error(
          `Cannot enable dish "${existingDish.name}": Insufficient ingredient stock - ${missingList}`
        );
      }
    }

    return await this.dishRepository.updateAvailability(
      id,
      isAvailable,
      reason,
      staffId
  ...
  async addIngredientToDish(dishId, ingredientData) {
    return await this.dishIngredientService.addIngredientToDish(
      dishId,
      ingredientData
    );
  }
  ...
  async updateDishIngredient(dishId, ingredientId, updateData) {
    return await this.dishIngredientService.updateDishIngredient(
      dishId,
      ingredientId,
      updateData
    );
  ...
  async removeIngredientFromDish(dishId, ingredientId) {
    return await this.dishIngredientService.removeIngredientFromDish(
      dishId,
      ingredientId
    );
  }
  ...
  async calculateDishRating(dishId) {
    try {
      const ratings = await DishRating.aggregate([
        {
      ...
      console.error("Error calculating dish rating:", error);
      return { average_rating: 0, total_reviews: 0 };
    }
  }
  ...
  async formatDishResponse(dish) {
    ...
    const dishId = dishObj._id || dishObj.id;
    const ratingData = await this.calculateDishRating(dishId);
