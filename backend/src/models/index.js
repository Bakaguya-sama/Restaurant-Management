...
const CustomerSchema = new Schema({
  membership_level: {
    type: String,
    enum: ["regular", "bronze", "silver", "gold", "platinum", "diamond"],
    default: "regular",
  },
...
const TableSchema = new Schema({
  ...
  status: {
    type: String,
    enum: ["free", "occupied", "reserved", "dirty", "broken"],
    default: "free",
  },
  created_at: { type: Date, default: Date.now },
  brokenReason: String,
...
const ComplaintSchema = new Schema({
  ...
  category: {
    type: String,
    enum: ["food", "service", "cleanliness", "other"],
    default: "other",
  },
  status: {
    type: String,
...
const IngredientSchema = new Schema({
  ...
  expiry_status: {
    type: String,
    enum: ["valid", "near_expiry", "expired"],
    default: "valid",
  },
  created_at: { type: Date, default: Date.now },
  updated_at: { type: Date, default: Date.now },
...
const DineInByCustomer = OrderSchema.discriminator(
  "dine-in-customer",
  new Schema({
    table_id: { type: Schema.Types.ObjectId, ref: "Table", required: true },
  })
...
const TakeawayByCustomer = OrderSchema.discriminator(
  "takeaway-customer",
  new Schema({})
);

const DineInByWaiter = OrderSchema.discriminator(
  "dine-in-waiter",
  new Schema({
    table_id: { type: Schema.Types.ObjectId, ref: "Table", required: true },
    staff_id: { type: Schema.Types.ObjectId, ref: "User", required: true }, // Waiter
...
const TakeawayByStaff = OrderSchema.discriminator(
  "takeaway-staff",
  new Schema({
    staff_id: { type: Schema.Types.ObjectId, ref: "User", required: true }, // Waiter
  })
...
const InvoiceSchema = new Schema({
  ...
  // Points applied by customer at checkout
  customer_selected_points: { type: Number, default: 0 },
  // Monetary value discounted because of points
  points_discount: { type: Number, default: 0 },
  total_amount: { type: Number, required: true },
  payment_method: {
    type: String,
    enum: ["cash", "card", "transfer", "e-wallet"],
  }, // Optional - set by cashier at payment time
  payment_status: {
    type: String,
...
const ViolationSchema = new Schema({
  ...
  violation_type: {
    type: String,
    enum: ["no_show", "late_cancel", "property_damage", "other"],
    default: "no_show",
  },
  // resolution: String,
  // resolved_at: Date,
...
const ReservationDetail = mongoose.model(
  "ReservationDetail",
...
const StockImportDetail = mongoose.model(
  "StockImportDetail",
  StockImportDetailSchema
);
...
const InvoicePromotion = mongoose.model(
  "InvoicePromotion",
  InvoicePromotionSchema
);
...
const StockExportDetail = mongoose.model(
  "StockExportDetail",
  StockExportDetailSchema
);
